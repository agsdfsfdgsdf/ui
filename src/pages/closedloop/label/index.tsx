...
import type { DictTypeType, DictTypeListParams } from './data';
import {
  getDictTypeList,
  getDict,
  removeDictType,
...
const DictTypeTableList: React.FC = () => {
  ...
  const columns: ProColumns<DictTypeType>[] = [
    ...
    },
    {
      ...
      valueType: 'text',
      render: (dom, record) => {
        if (access.hasPerms('system:dict:list')) {
          return (
            <a
              onClick={() => {
                history.push(`/system/dictData/index/${record.dictId}`);
              }}
            >
              {dom}
            </a>
    ...
    },
    {
      ...
      render: (_, record) => [
        <Button
          ...
          size="small"
          key="edit"
          hidden={!access.hasPerms('system:dict:edit')}
          onClick={() => {
        ...
        <Button
          ...
          danger
          key="batchRemove"
          hidden={!access.hasPerms('system:dict:remove')}
          onClick={async () => {
            Modal.confirm({
              title: '删除',
              content: '确定删除该项吗？',
        ...
        <ProTable<DictTypeType>
          ...
          toolBarRender={() => [
            <Button
              type="primary"
              key="add"
              hidden={!access.hasPerms('system:dict:add')}
              onClick={async () => {
            ...
            <Button
              type="primary"
              key="remove"
              hidden={selectedRowsState?.length === 0 || !access.hasPerms('system:dict:remove')}
              onClick={async () => {
            ...
            <Button
              type="primary"
              key="export"
              hidden={!access.hasPerms('system:dict:export')}
              onClick={async () => {
                handleExport();
              }}
            >
