...
import { addMirrorData } from '../server';
...
const DataUpload: React.FC<DataUploadProps> = (props: DataUploadProps) => {
  const mirrordata_key = 'mirrordata_upload';
    ...
    try {
      ...
      if (oss && oss.data && oss.data.securityToken) {
        ...
        BizFromUploadFile.uploadFile(
          BizFromUploadFile.getFileList(mirrordata_key).filter(
  ...
  useEffect(() => {
    ...
    const rowFiles = BizFromUploadFile.getFileList(mirrordata_key);
    ...
    const dataSub1 = DataBus.subscribe({
      ...
      callback: async (data: UploadFileInfo) => {
        ...
        if (data && data.upkey === mirrordata_key) {
          console.log('上传完成', data, creator);
          const result = await addMirrorData({
