...
import type { MirrorType, MirrorTypeParam } from '../data';
import { getMirrorDataList } from '../server';
...
export type DataTableProps = {
  onRef?: any;
  onDownload: (data: MirrorType) => void;
  onDelete: (data: MirrorType) => void;
  onSelected: (datas: MirrorType[]) => void;
  onShowInfo: (data: MirrorType, allData: MirrorType[]) => void;
};
...
const MirrorTable = (props: DataTableProps) => {
  const [mirrorData, setMirrorData] = useState<MirrorType[]>([]);
  ...
  const loadList = useCallback(
    (parent?: string | null) => {
      ...
      const params =
        parent1 && parent1 !== '' && parent1 !== 'imageRoot'
          ? ({ parent: parent1, ...pageInfo1 } as MirrorTypeParam)
          : ({ ...pageInfo1 } as MirrorTypeParam);

      getMirrorDataList(params).then((res) => {
        onSelected([]);
        setSelectedRows([]);
  ...
  const sharedOnCell = (record: MirrorType) => {
  ...
  const columns: ProColumns<MirrorType>[] = [
    {
      title: '文件名',
      dataIndex: 'name',
      valueType: 'text',
    ...
    },
    {
      ...
      render: (_, record) => [
        <Button
          ...
          size="small"
          key="download"
          hidden={!access.hasPerms('closeloop:mirrordata:download')}
          onClick={(e) => {
            onDownload(record);
            e.stopPropagation();
          }}
        ...
        <Button
          ...
          size="small"
          key="delete"
          hidden={!access.hasPerms('closeloop:mirrordata:delete')}
          onClick={(e) => {
            console.log(record)
            onDelete(record);
            e.stopPropagation();
      ...
      <ProTable<MirrorType>
        ...
        pagination={{
          ...
          onChange: (page: number, pageSize: number) => {
            ...
            getMirrorDataList(
              parentProLoadData && parentProLoadData !== '' && parentProLoadData !== 'imageRoot'
                ? ({ parent: parentProLoadData, ...pageInfo1 } as MirrorTypeParam)
                : ({ ...pageInfo1 } as MirrorTypeParam),
            ).then((res) => {
              onSelected([]);
              setSelectedRows([]);
        ...
        rowSelection={{
          ...
          onChange: (selectedRowKeys: React.Key[], selectedRows: MirrorType[]) => {
              ...
              <Table.Summary.Row>
                ...
                <Table.Summary.Cell index={0} colSpan={11}>
                  <Breadcrumb style={{ color: 'blue' }} separator="/">
                    ...
                    {paths &&
                      paths
                        .filter((path) => path.indexOf('imageRoot') === -1)
                        .map((path, index0) => {
                          return (
                            <Breadcrumb.Item
