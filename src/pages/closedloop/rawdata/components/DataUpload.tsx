...
const DataUpload: React.FC<DataUploadProps> = (props: DataUploadProps) => {
  const rawdata_key = 'rawdata_upload';
  ...
  const onFinish = async () => {
    try {
      ...
      if (!values || !values.project || values.project === '') {
        message.error('未检测到项目名称，请手动输入！');
        return;
      }
      if (!values || !values.vin || values.vin === '') {
        message.error('未检测到车辆VIN，请手动输入！');
        return;
      }
      ...
      if (oss && oss.data && oss.data.securityToken) {
        ...
        BizFromUploadFile.uploadFile(
          BizFromUploadFile.getFileList(rawdata_key).filter(
  ...
  useEffect(() => {
    queryCurrentUserInfo().then((result) => {
      console.log('Gunson -------------- ', result?.data?.user?.userName);
      creator = result?.data?.user?.userName;
    });

    const rowFiles = BizFromUploadFile.getFileList(rawdata_key);
    ...
    const dataSub1 = DataBus.subscribe({
      ...
      callback: async (data: UploadFileInfo) => {
        ...
        if (data && data.upkey === rawdata_key) {
          console.log('上传完成', data, creator);
          const result = await addRawData({
    ...
    <div>
      <div style={{ padding: '10px 16px', clear: 'both', display: props.show ? 'block' : 'none' }}>
        <Form form={form} layout="horizontal" onFinish={onFinish}>
          <Form.Item>
            <Alert
              message="提示：上传压缩包文件成功后，后台需解压获取目录结构方可显示在原始数据管理列表页，请刷新页面耐心等待！"
              type="info"
              showIcon
              style={{ float: 'left' }}
            />
          ...
          <Form.Item
            ...
            wrapperCol={{ span: 14 }}
          >
            <Input placeholder="请输入项目名称！" disabled={disable} />
          </Form.Item>
          <Form.Item name="vin" label="车辆VIN：" labelCol={{ span: 2 }} wrapperCol={{ span: 14 }}>
            <Input placeholder="请输入车辆VIN！" disabled={disable} />
          </Form.Item>
          <Form.Item
            name="files"
            label="待上传文件："
