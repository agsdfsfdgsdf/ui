...
const BizFromUploadFile = {
  ...
  uploadFile: (fileInfo: UploadFileInfo) => {
    ...
    const dir = (fileInfo.upkey === 'dataset_upload' ? fileInfo.fileDir :
      fileInfo.upkey === 'rawdata_upload' ? 'zipRoot/collectRoot' :
        fileInfo.upkey === 'mirrordata_upload' ? 'imageRoot' :
          fileInfo.upkey === 'modeldata_upload' ? 'modelRoot' :
            'zipRootFail');
    ...
    return ossClient
      ...
      .then((res) => {
        ...
        if (waitList && waitList.length > 0) {
          BizFromUploadFile.uploadFile(waitList[0]);
        }
      })
      .catch((err) => {
        console.error(err);
        if (ossClient && ossClient.isCancel()) {
          ...
          if (err.name.toLowerCase().indexOf('connectiontimeout') !== -1) {
            ...
            if (retryCount < retryCountMax) {
              ...
              BizFromUploadFile.uploadFile(fileInfo);
            }
          }
        }
      });
...
export { BizFromUploadFile, UploadStatus };
