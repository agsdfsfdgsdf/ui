...
export async function getJobList(params?: JobListParams) {
  ...
  return request(getUrl(`/api/monitor/job/list?${queryString}`), {
    data: params,
    method: 'GET',
    headers: {
...
export function getJob(jobId: number) {
  return request(getUrl(`/api/monitor/job/${jobId}`), {
    method: 'GET',
  });
}
...
export async function addJob(params: JobType) {
  return request(getUrl('/api/monitor/job'), {
    method: 'POST',
    data: params,
  });
}
...
export async function removeJob(ids: string) {
  return request(getUrl(`/api/monitor/job/${ids}`), {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json;charset=UTF-8',
    },
...
export async function runJob(jobId: number, jobGroup: string) {
  ...
  return request(getUrl('/api/monitor/job/run'), {
    method: 'PUT',
    data: job,
  });
}
