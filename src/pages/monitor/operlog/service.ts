...
export async function getOperlogList(params?: OperlogListParams) {
  ...
  return request(getUrl(`/api/monitor/operlog/list?${queryString}`), {
    data: params,
    method: 'GET',
    headers: {
...
export function getOperlog(operId: number) {
  return request(getUrl(`/api/monitor/operlog/${operId}`), {
    method: 'GET',
  });
}
...
export async function addOperlog(params: OperlogType) {
  return request(getUrl('/api/monitor/operlog'), {
    method: 'POST',
    data: params,
  });
}
...
export async function removeOperlog(ids: string) {
  return request(getUrl(`/api/monitor/operlog/${ids}`), {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json;charset=UTF-8',
    },
...
export function exportOperlog(params?: OperlogListParams) {
  return downLoadXlsx(
    getUrl(`/api/monitor/operlog/export`),
    { params },
...
export async function cleanOperlog() {
  return request(getUrl('/api/monitor/operlog/clean'), {
    method: 'DELETE',
  });
}
