...
export async function getConfigList(params?: ConfigListParams) {
  ...
  return request(getUrl(`/api/system/config/list?${queryString}`), {
    data: params,
    method: 'GET',
    headers: {
...
export function getConfig(configId: number) {
  return request(getUrl(`/api/system/config/${configId}`), {
    method: 'GET',
  });
}
...
export async function addConfig(params: ConfigType) {
  return request(getUrl('/api/system/config'), {
    method: 'POST',
    data: params,
  });
}
...
export async function removeConfig(ids: string) {
  return request(getUrl(`/api/system/config/${ids}`), {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json;charset=UTF-8',
    },
...
export function exportConfig(params?: ConfigListParams) {
  return downLoadXlsx(
    getUrl(`/api/system/config/export`),
    { params },
