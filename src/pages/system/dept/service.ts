...
export async function getDeptList(params?: DeptListParams) {
  ...
  return request(getUrl(`/api/system/dept/list?${queryString}`), {
    data: params,
    method: 'GET',
    headers: {
...
export function getDeptListExcludeChild(deptId: number) {
  return request(getUrl(`/api/system/dept/list/exclude/${deptId}`), {
    method: 'get',
  });
}
...
export function getDept(deptId: number) {
  return request(getUrl(`/api/system/dept/${deptId}`), {
    method: 'GET',
  });
}
...
export async function addDept(params: DeptType) {
  return request(getUrl('/api/system/dept'), {
    method: 'POST',
    data: params,
  });
}
...
export async function removeDept(ids: string) {
  return request(getUrl(`/api/system/dept/${ids}`), {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json;charset=UTF-8',
    },
...
export function getTreeList(params: any): Promise<DataNode[]> {
  return new Promise((resolve) => {
    const queryString = new URLSearchParams(params).toString();
    request(getUrl(`/api/system/dept/treeselect?${queryString}`), {
      method: 'get',
      headers: {
        'Content-Type': 'application/json;charset=UTF-8',
      },
