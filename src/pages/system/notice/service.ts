...
export async function getNoticeList(params?: NoticeListParams) {
  ...
  return request(getUrl(`/api/system/notice/list?${queryString}`), {
    data: params,
    method: 'GET',
    headers: {
...
export function getNotice(noticeId: number) {
  return request(getUrl(`/api/system/notice/${noticeId}`), {
    method: 'GET',
  });
}
...
export async function addNotice(params: NoticeType) {
  return request(getUrl('/api/system/notice'), {
    method: 'POST',
    data: params,
  });
}
...
export async function removeNotice(ids: string) {
  return request(getUrl(`/api/system/notice/${ids}`), {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json;charset=UTF-8',
    },
...
export function exportNotice(params?: NoticeListParams) {
  return downLoadXlsx(
    getUrl(`/api/system/notice/export`),
    { params },
