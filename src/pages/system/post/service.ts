...
export async function getPostList(params?: PostListParams) {
  ...
  return request(getUrl(`/api/system/post/list?${queryString}`), {
    data: params,
    method: 'GET',
    headers: {
...
export function getPost(postId: number) {
  return request(getUrl(`/api/system/post/${postId}`), {
    method: 'GET',
  });
}
...
export async function addPost(params: PostType) {
  return request(getUrl('/api/system/post'), {
    method: 'POST',
    data: params,
  });
}
...
export async function removePost(ids: string) {
  return request(getUrl(`/api/system/post/${ids}`), {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json;charset=UTF-8',
    },
